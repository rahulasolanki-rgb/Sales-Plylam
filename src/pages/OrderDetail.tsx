...
import { ArrowLeft, Share2, Download, Package, Truck, Check, CreditCard, Inbox, CheckCircle2 } from "lucide-react";
      ...
      <div className="p-6 space-y-6">
            ...
            {order.items?.map((item, i) => (
              <div key={i} className="flex items-center justify-between p-4 bg-background-light rounded-xl border border-gray-200">
                <div className="flex-1">
                  <p className="text-text-dark font-bold text-sm">{item.name}</p>
                  <p className="text-text-dark/60 text-xs">{item.quantity} Units • ₹{item.price}/ea</p>
                </div>
                <p className="text-text-dark font-bold">₹{(item.price * item.quantity).toLocaleString()}</p>
              </div>
            ))}
          </div>
        </section>
        ...
        <div className="grid grid-cols-1 gap-4">
          <div className="p-4 rounded-xl border border-gray-200">
            <p className="text-text-dark/50 text-[10px] font-bold uppercase tracking-widest mb-2">Shipping Address</p>
            <p className="text-text-dark text-xs leading-relaxed font-medium">
              Oakridge Construction<br/>
